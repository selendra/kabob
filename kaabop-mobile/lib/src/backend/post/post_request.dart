...
//   "address": "5CyvZsXQAUNKYH6f38vCku9bXkWVAxaAwksFYRDpGJYcx4Vv"
// }' http://localhost:3000/:service/contract/validate
// secretKey
  ...
  // Confirm User Account By Phone Number
  Future<_http.Response> forgetPassword(
      ForgetModel forgetM, String value) async {
    _backend.bodyEncode = json.encode({
      forgetM.key: value,
    });
  ...
  Future<_http.Response> resetPass(
      ForgetModel forgetM, String value, String endpoint) async {
    _backend.bodyEncode = json.encode({
  ...
  Future<_http.StreamedResponse> upLoadImage(
    ...
    request.headers.addAll(_backend.conceteHeader(
        "Authorization", "Bearer ${_backend.token['TOKEN']}"));
